...
EVENT_ICONS = {
    "earnings":"📊","guidance":"🎯","M&A":"🤝","merger":"🤝","acquisition":"🤝",
    "regulation":"⚖️","litigation":"🏛️","executive_change":"👤","product_launch":"🚀",
    "analyst_rating":"⭐","partnership":"🔗","macro":"🌐","market_trend":"📈","other":"📌",
}


def _card_html(e: dict) -> str:
    color = PRIORITY_COLORS.get(e.get("importance_label","Low"), "#22c55e")
    icon = EVENT_ICONS.get(e.get("event_type","other"), "📌")
    sent_emoji = SENTIMENT_EMOJI.get(e.get("sentiment","neutral"), "◆")
    tickers = ", ".join(e.get("tickers_affected",[]))
    bullets = "".join(f"<li style='margin:4px 0;color:#ccc'>{b}</li>" for b in e.get("key_bullets",[]))
...
<div style="border:1px solid #333;border-left:3px solid {color};border-radius:8px;padding:16px;margin:10px 0;background:#1a1a1a">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
    <strong style="color:#f0f0f0;font-size:14px">{icon} {e.get("event_title","")}</strong>
    <span style="color:{color};font-size:12px;font-weight:700;white-space:nowrap;margin-left:12px">{e.get("importance_label","")}</span>
  </div>
  ...
  <div style="background:#0f2a1a;border:1px solid #1a4a2a;border-radius:6px;padding:10px;font-size:13px;color:#86efac">
    <strong>💼 Investment Perspective</strong><br>{e.get("investment_impact","")}
  </div>
  {flag_html}
...
def notification_agent(state: PipelineState) -> PipelineState:
    ...
    high   = [e for e in events if e.get("importance_label") == "High"]
    medium = [e for e in events if e.get("importance_label") == "Medium"]
    low    = [e for e in events if e.get("importance_label") == "Low"]
...
<div style="margin-top:32px;font-size:11px;color:#444;text-align:center">FinTel · OpenAI GPT-4o + LangGraph · Not financial advice</div>
    ...
    for label, evts in [("HIGH", high), ("MEDIUM", medium), ("LOW", low)]:
        for e in evts:
            text_lines += [f"[{label}] {e.get('event_title','')}", e.get("tldr",""), ""]
