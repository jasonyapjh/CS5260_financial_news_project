...
with st.sidebar:
    ...
    st.caption("No other API keys needed — news is fetched from free public sources.")
    ...
    st.divider()
    st.markdown(
        "<span style='font-size:11px;color:#4a5a6a;font-family:JetBrains Mono'>OpenAI GPT-4o · LangGraph<br>Free RSS Sources · Streamlit</span>",
        unsafe_allow_html=True,
    )
...
if st.session_state.running and not st.session_state.pipeline_result:
    ...
    _originals = {
        ...
        "market_data":   _mda.market_data_agent,
    ...
    def make_wrapper(fn, step_idx):
        def wrapper(state):
            ...
            overall_progress.progress(step_idx / 8, text=f"Agent {step_idx}/7 complete")
            return result
        return wrapper

    _wa.watchlist_agent     = make_wrapper(_originals["watchlist"],     1)
    _mda.market_data_agent  = make_wrapper(_originals["market_data"],   2)
    _ra.retrieval_agent     = make_wrapper(_originals["retrieval"],     3)
    _fa.filter_agent        = make_wrapper(_originals["filter"],        4)
    _ca.clustering_agent    = make_wrapper(_originals["clustering"],    5)
    _sa.summarization_agent = make_wrapper(_originals["summarization"], 6)
    _rka.ranking_agent      = make_wrapper(_originals["ranking"],       7)
    _na.notification_agent  = make_wrapper(_originals["notification"],  8)
    ...
    try:
        ...
        _mda.market_data_agent  = _originals["market_data"]
...
# ══════════════════════════════════════════════════════════════════════
    ...
    c1.metric("📥 Fetched",    result.get("raw_article_count", 0))
    c2.metric("🧹 Filtered",   result.get("clean_article_count", 0))
    c3.metric("🗂 Clusters",   len(result.get("event_clusters", [])))
    c4.metric("📋 Events",     len(all_events))
    c5.metric("🔴 High",       len(high))
    c6.metric("🟠 Medium",     len(medium))
    ...
    # ── Filter tabs ──────────────────────────────────────────────────
    tab_all, tab_high, tab_med, tab_low, tab_mkt, tab_digest, tab_logs = st.tabs([
    ...
    def render_event_card(e: dict):
        ...
        tickers_str = "  ".join(f"`{t}`" for t in [e.get("ticker", "")])
        score = e.get("importance_score", 0)

        header = (
            f"{icon} **{e.get('representative_headline', '') or e.get('tldr', '')[:80]}** &nbsp;&nbsp;"
        ...
        with st.expander(f"{icon}  {e.get('representative_headline', '') or e.get('tldr', '')[:80]}  ·  {label}  ·  {sent.capitalize()}", expanded=False):
            # Meta row
            col_meta, col_score = st.columns([4, 1])
            with col_meta:
                rank = e.get("rank_overall", "")
                rank_str = f"#{rank}" if rank else ""
                rep_src = e.get("representative_source", "")
                st.markdown(
                    f"**Ticker:** {tickers_str} &nbsp;·&nbsp; "
                    f"**Type:** `{etype.replace('_',' ').title()}` &nbsp;·&nbsp; "
                    f"**{e.get('article_count', 0)} articles** &nbsp;·&nbsp; "
                    f"**Rep. source:** {rep_src}"
                    + (f" &nbsp;·&nbsp; **Rank {rank_str}**" if rank_str else "")
                )
            with col_score:
                st.markdown(
                    f"<div style='text-align:right;font-family:JetBrains Mono;font-size:22px;"
                    f"font-weight:700;color:{pcolor}'>{score:.2f}<span style='font-size:12px;color:#4a5a6a'>/1.0</span></div>",
                    unsafe_allow_html=True,
                )
            ...
            # Market context panel (if available)
            ticker_key = e.get("ticker", "")
            mctx = result.get("market_context", {}).get(ticker_key, {})
            if mctx and not mctx.get("_error") and mctx.get("last_price"):
                ccy = mctx.get("currency", "")
                p1d = mctx.get("price_change_1d")
                p5d = mctx.get("price_change_5d")
                vr  = mctx.get("volume_ratio")
                ar  = mctx.get("analyst_rating")
                tp  = mctx.get("target_price")
                p1d_str = f"{p1d:+.1f}%" if p1d is not None else "—"
                p5d_str = f"{p5d:+.1f}%" if p5d is not None else "—"
                p1d_col = "#22c55e" if (p1d or 0) >= 0 else "#ef4444"
                p5d_col = "#22c55e" if (p5d or 0) >= 0 else "#ef4444"
                vr_flag = " 🔥" if (vr or 0) > 2.0 else ""
                st.markdown(
                    f"""<div style='background:#0a1020;border:1px solid #1e2d3d;border-radius:6px;
                    padding:10px 14px;margin:10px 0;display:flex;gap:24px;flex-wrap:wrap;font-family:JetBrains Mono;font-size:12px'>
                    <span style='color:#4a5a6a'>💰 Price</span>
                    <span style='color:#d4dde8;font-weight:700'>{ccy} {mctx['last_price']:.2f}</span>
                    <span style='color:#4a5a6a'>1D</span>
                    <span style='color:{p1d_col};font-weight:700'>{p1d_str}</span>
                    <span style='color:#4a5a6a'>5D</span>
                    <span style='color:{p5d_col};font-weight:700'>{p5d_str}</span>
                    {f"<span style='color:#4a5a6a'>Vol</span><span style='color:#d4dde8;font-weight:700'>{vr:.1f}x{vr_flag}</span>" if vr else ""}
                    {f"<span style='color:#4a5a6a'>Analyst</span><span style='color:#00c8ff;font-weight:700'>{ar}</span>" if ar else ""}
                    {f"<span style='color:#4a5a6a'>Target</span><span style='color:#d4dde8;font-weight:700'>{ccy} {tp:.2f}</span>" if tp else ""}
                    </div>""",
                    unsafe_allow_html=True,
                )
            ...
            signals = e.get("scoring_signals", {})
            if signals:
                with st.expander("📊 Scoring breakdown", expanded=False):
                    sc1, sc2, sc3, sc4 = st.columns(4)
                    sc1.metric("Event Type",    f"{signals.get('event_type_weight', 0):.2f}",   help="Weight: 40% of total score")
                    sc2.metric("Corroboration", f"{signals.get('corroboration_score', 0):.2f}", help=f"{signals.get('corroboration_count',0)} unique sources · Weight: 25%")
                    sc3.metric("Novelty",       f"{signals.get('novelty_score', 0):.2f}",       help=f"{e.get('article_count',0)} articles / 8 · Weight: 20%")
                    sc4.metric("Credibility",   f"{signals.get('credibility_score', 0):.2f}",   help="Avg source tier score · Weight: 15%")
                    bonuses = []
                    if signals.get("volume_bonus", 0) > 0:
                        bonuses.append("📈 Abnormal volume +0.05")
                    if signals.get("earnings_proximity_bonus", 0) > 0:
                        bonuses.append("📅 Near earnings date +0.05")
                    if bonuses:
                        st.caption("  ·  ".join(bonuses))
                    st.caption(
                        f"Formula: {signals.get('event_type_weight',0):.2f}×0.40 + "
                        f"{signals.get('corroboration_score',0):.2f}×0.25 + "
                        f"{signals.get('novelty_score',0):.2f}×0.20 + "
                        f"{signals.get('credibility_score',0):.2f}×0.15 "
                        f"= {e.get('importance_score',0):.4f}"
                    )
    ...
    with tab_mkt:
        ...
        if not market_context:
            ...
            for ticker, ctx in market_context.items():
                if ctx.get("_error"):
                    st.caption(f"{ticker}: market data unavailable ({ctx['_error']})")
                    continue
                ccy = ctx.get("currency", "")
                p   = ctx.get("last_price")
                p1d = ctx.get("price_change_1d")
                p5d = ctx.get("price_change_5d")
                vr  = ctx.get("volume_ratio")
                ar  = ctx.get("analyst_rating")
                tp  = ctx.get("target_price")
                ed  = ctx.get("earnings_date")
                with st.expander(f"**{ticker}** — {ccy} {p:.2f}" if p else f"**{ticker}**", expanded=True):
                    mc1, mc2, mc3, mc4 = st.columns(4)
                    mc1.metric("Price", f"{ccy} {p:.2f}" if p else "—")
                    mc2.metric("1-Day", f"{p1d:+.1f}%" if p1d is not None else "—",
                               delta=f"{p1d:+.1f}%" if p1d is not None else None,
                               delta_color="normal")
                    mc3.metric("5-Day", f"{p5d:+.1f}%" if p5d is not None else "—",
                               delta=f"{p5d:+.1f}%" if p5d is not None else None,
                               delta_color="normal")
                    mc4.metric("Volume Ratio", f"{vr:.1f}x" if vr else "—",
                               help="Today vs 30-day avg. >2.0 = abnormal")
                    if ar or tp or ed:
                        mc5, mc6, mc7 = st.columns(3)
                        mc5.metric("Analyst Rating", ar or "—")
                        mc6.metric("Target Price",   f"{ccy} {tp:.2f}" if tp else "—")
                        mc7.metric("Next Earnings",  ed or "—")
...
# ══════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        "OpenAI GPT-4o · LangGraph StateGraph · 8 agents · No news API key needed</p>",
        unsafe_allow_html=True,
    )

    steps = [
        ("Watchlist & Context Agent",    "Resolves tickers to company names, aliases, sector; generates company + industry query bundles"),
        ("Market Data Agent",            "Fetches price, volume ratio, analyst rating, earnings date per ticker (Yahoo Finance)"),
        ("News Retrieval Agent",         "Parallel fetch from Yahoo Finance RSS, Google News RSS, Reuters RSS, Finviz & Seeking Alpha — no API key"),
        ("Noise Filter & Dedup Agent",   "Pass 1: URL/headline dedup + recency filter · Pass 2: GPT-4o semantic deduplication"),
        ("Event Clustering Agent",       "Keyword-rule event typing + GPT-4o grouping into typed clusters per ticker"),
        ("Impact Summarization Agent",   "GPT-4o: tldr, key_facts, impact + market context injection + verification pass"),
        ("Importance Ranking Agent",     "Formula score (0–1): event_type×40% + corroboration×25% + novelty×20% + credibility×15%"),
        ("Digest Packaging Agent",       "Packages ranked events into digest JSON + styled HTML email; sorts by rank_overall"),
    ]

    STEP_COLORS = ["#00c8ff","#00b4d8","#5a9fd4","#9b5de5","#f72585","#ef4444","#f97316","#22c55e"]
    ...
    st.divider()
    st.info("👈  Enter your API keys and watchlist in the sidebar, then click **Run Pipeline**.")
